...
(function () {
  ...
  /* ════════════════════════════════════════════════════════════
     ÉTUDES DE LA LITTÉRATURE — MATCHING STRICT PAR RÉSULTAT
     On n'utilise plus un matching large par tags HER2.
     On utilise uniquement la table base_etudes.json > matching_resultats.
  ════════════════════════════════════════════════════════════ */

  function idsEtudesPourResultats(traitementsRecommandes) {
    var mapping = {};
    if (window.BASE_ETUDES_MAPPING) mapping = window.BASE_ETUDES_MAPPING;
    // Normalement, le mapping vient de base_etudes.json.
    // Sécurité si la base est chargée dans la variable globale "baseEtudes".
    return traitementsRecommandes.reduce(function(acc, traitement) {
      var ids = mapping[traitement] || [];
      ids.forEach(function(id) {
        if (acc.indexOf(String(id)) === -1) acc.push(String(id));
      });
      return acc;
    }, []);
  }

  function mappingResultatsDepuisBase() {
    // Le fichier base_etudes.json peut contenir une clé "matching_resultats".
    // Comme app.js ne garde que "etudes" actuellement, on récupère cette clé au chargement.
    return {};
  }
  ...
  function renderEtudes(traitementsRecommandes, profilTexte) {
    ...
    var ids = [];
    traitementsRecommandes.forEach(function(traitement) {
      var found = etudes.filter(function(e) {
        return (e.resultats_atlas || []).indexOf(traitement) !== -1;
      });
    ...
    intro.innerHTML =
      '<strong>Résultat(s) utilisé(s) pour sélectionner les études :</strong> ' +
      traitementsRecommandes.map(escapeHtml).join(' · ');
    section.appendChild(intro);

    if (!ids.length) {
      var p = document.createElement('p');
      p.style.cssText = 'color:#636e72;font-size:13px;margin-top:12px;';
      p.textContent = 'Aucune étude de la base actuelle n’est associée précisément à ce résultat.';
      section.appendChild(p);
      return;
    }

    var selected = etudes.filter(function(e) {
      return ids.indexOf(String(e.id)) !== -1;
    }).sort(function(a, b) {
      return niveauRank(b.niveau_preuve) - niveauRank(a.niveau_preuve);
    });

    selected.forEach(function(etude) {
      section.appendChild(creerCarteEtude(etude));
    });
  }
  ...
  function creerCarteEtude(etude) {
    var card = document.createElement('div');
    ...
    var lien = etude.lien || '';
    var resultats = etude.resultats_atlas || [];
    ...
    var html =
      '<div style="display:flex;justify-content:space-between;gap:16px;align-items:flex-start;">' +
        '<div style="flex:1;">' +
          ...
          '</div>' +
          (resultats.length ? '<div style="font-size:12px;color:#636e72;margin-bottom:10px;">Associée à : ' + escapeHtml(resultats.join(', ')) + '</div>' : '') +
